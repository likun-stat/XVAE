...
for (t in 1:niter) {
  ...
  if(t==5000) { learning_rate <- -5e-12; alpha_v <- 0.8}
  if(t==10000) { learning_rate <- -1e-11; alpha_v <- 0.7}
  if(t==20000) { learning_rate <- -4e-11; alpha_v <- 0.6}
  if(t==80000) { learning_rate <- -1e-10; alpha_v <- 0.4}
  if(t==100000) { learning_rate <- -5e-10; alpha_v <- 0.4}
...
save(better_Z_approx, n.sim, y_star, station1_Simulations, file="../simulation_VAE_results.RData" )
