...
# start_time <- Sys.time()
for (t in 1:niter) {
  if(t==1000) { learning_rate <- -2e-14; alpha_v <- 0.9}
  if(t==5000) { learning_rate <- -2e-14; alpha_v <- 0.9}
  if(t==10000) { learning_rate <- -3e-14; alpha_v <- 0.7}
  if(t==20000) { learning_rate <- -4e-14; alpha_v <- 0.6}
  if(t==80000) { learning_rate <- -6e-14; alpha_v <- 0.5}
  if(t==100000) { learning_rate <- -1e-12; alpha_v <- 0.4}
  if(t==120000) { learning_rate <- -1e-10; alpha_v <- 0.4}
...
#      file="../simulation_VAE_results.RData" )

which_iter <- which_iter+1
save(Theta_t_estimations, file=paste0("../theta_flex_", which_iter, ".RData"))


## -------- time 1 --------
ind <- 1
range_t <- range(X[,ind])
q25 <- 0.2#quantile(X[,ind], 0.25)
q75 <- 6.4#quantile(X[,ind], 0.75)
...
plt3 <- spatial_map(stations, var=station1_Simulations[,1], pal = pal,ylab = ylab,
...
plt3
# ggsave("~/Desktop/GEV-GP_VAE/Figures/sim_flex.png",width = 4.85, height = 4.9)
...
plt31
# ggsave("~/Desktop/GEV-GP_VAE/Figures/emu_flex.png",width = 4.3, height = 4.9)

load("~/Desktop/GEV-GP_VAE/Flex_hetgp_emulation.RData")
source("utils.R")
stations <- data.frame(stations)
tmp_emu_gp <- p_org$mean
plt32 <- spatial_map(stations, var=exp(tmp_emu_gp+rnorm(n=n.s, sd=sqrt(p_org$nugs))), pal = pal,
...
plt32
ggsave("~/Desktop/GEV-GP_VAE/Figures/het_flex.png",width = 5.4, height = 4.9)


ind <- 1

png(filename="~/Desktop/GEV-GP_VAE/Figures/qq_emu_flex.png",width = 400, height = 380)
par(mar = c(5.1, 6.1, 4.1, 2.1))
extRemes::qqplot(X[,ind], station1_Simulations[,24]*0.9, regress = FALSE,  
                 xlab=expression(paste('Simulated ', X[1])), 
                 ylab=bquote(atop(bold("Model III"), paste('Emulated ', X[1]))), cex.lab = 1.2,
                 main='extVAE', xlim=c(0,40), ylim=c(0,40))
dev.off()


png(filename="~/Desktop/GEV-GP_VAE/Figures/qq_het_flex.png",width = 400, height = 380)
par(mar = c(5.1, 6.1, 4.1, 2.1))
extRemes::qqplot(X[,ind], exp(tmp_emu_gp+rnorm(n=n.s, sd=sqrt(p_org$nugs))), regress = FALSE,
                 ...
                 main='hetGP', xlim=c(0,40), ylim=c(0,40))
dev.off()
...
save(CRPS_by_loc, MSPE, file="~/Desktop/GEV-GP_VAE/Figures/CRPS_MSPE_flex_true_knots.RData")
...
ylab = expression(atop(bold("Model III"), chi[u]))


chi_plot(X,stations, station_Simulations_All, distance=0.5, legend = FALSE, ylab = ylab)
ggsave("~/Desktop/GEV-GP_VAE/Figures/flex_qqplot_0_5.png",width = 3.95, height = 3.95)

chi_plot(X,stations, station_Simulations_All, distance=2, legend = FALSE, show.axis.y = FALSE)
ggsave("~/Desktop/GEV-GP_VAE/Figures/flex_qqplot_2.png",width = 3.5, height = 3.95)

chi_plot(X,stations, station_Simulations_All, distance=5, show.axis.y = FALSE)
ggsave("~/Desktop/GEV-GP_VAE/Figures/flex_qqplot_5.png",width = 4.4, height = 3.95)
...
save(u_vec, Emp_ARE_ori, Emp_ARE, file="~/Desktop/GEV-GP_VAE/ARE_flex.RData")

load("~/Desktop/GEV-GP_VAE/ARE_flex.RData")
...
plt <- ggplot(dat[-(296:298),],aes(x=x,y=truth)) +
  ...
  scale_y_continuous(expand = c(0, 0), limits=c(0, sqrt(100/pi))) + 
...
plt
ggsave("~/Desktop/GEV-GP_VAE/Figures/ARE_flex.png",width = 3.5, height = 3.95)
