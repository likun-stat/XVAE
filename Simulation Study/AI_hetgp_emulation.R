...
library(hetGP)
start_time <- Sys.time()
fit <- mleHetGP(as.matrix(stations), X[,ind], covtype = "Matern5_2", lower = rep(0.05, 2),
                 upper = rep(10, 2), settings = list(linkThetas = "none"), maxit = 5e2)
end_time <- Sys.time()
...
save(X, stations, p_org, p, file = "~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/AI_hetgp_emulation.RData")


## All time replicates
emulation_AI <- matrix(NA, nrow=1900, ncol=100)
pred_AI <- matrix(NA, nrow=100, ncol=100)
for(iter in 1:100){
  cat('iter=',iter,'\n')
  ...
  p_org <- predict(x = as.matrix(stations), object = fit)
  emulation_AI[,iter] <- p_org$mean
  
  p_pred <- predict(x = as.matrix(stations_holdout), object = fit)
  pred_AI[,iter] <- p_pred$mean
  
}

save(fit, X, X_holdout, stations, emulation_AI, pred_AI, file="~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/HomAI.RData")


###### ---------------------------------------------------------------------- ######
###### ------------------------------ MSPE & CRPS --------------------------- ######
###### ---------------------------------------------------------------------- ######
load("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/HomAI.RData")

### -------- MSPE ---------
# MSPE <- colMeans((station_Simulations - X_holdout)^2)
tmp <- (pred_AI - X_holdout)^2
MSPE <- apply(tmp, 2, function(x) mean(x))
boxplot(MSPE,ylim=c(0,400))
...
for(loc in 1:100){
  ...
  cat("loc=", loc, "\n")
  for(t in 1:n.t){
    tmp_integrand <- function(z) ecdf_tmp(z) - I(pred_AI[loc,t]<=z) 
...
save(CRPS_by_loc, MSPE, file="~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/CRPS_MSPE_AI_hetGP.RData")



CRPS_by_loc_hetGP <- CRPS_by_loc; MSPE_hetGP <- MSPE
load("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/CRPS_MSPE_AI.RData")
