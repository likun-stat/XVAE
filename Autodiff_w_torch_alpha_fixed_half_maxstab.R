...
for (t in 1:niter) {
  ...
  if(t==5000) { learning_rate <- -1e-25; alpha_v <- 0.9}
...
station27_Simulations <- matrix(NA, nrow=n.s, ncol=n.sim)
...
for(iter in 1:n.sim){
  ...
  station27_Simulations[, iter] <- rfrechet(n.s,shape=1, location = m, scale = tau) * as_array((y_star[,27]))
  station100_Simulations[, iter] <- rfrechet(n.s,shape=1, location = m, scale = tau) * as_array((y_star[,100]))
}
save(better_Z_approx, n.sim, y_star, theta_t, station27_Simulations, file="../simulation_VAE_results_maxstat.RData" )


## -------- time 1 --------
ind <- 27
range_t <- range(X[,ind], station27_Simulations)
q25 <- quantile(X[,ind], 0.25)
q75 <- quantile(X[,ind], 0.95)

library(ggtext)
ylab = paste0("<span style='font-size: 16pt'>**Model  V**</span><br><span style='font-size: 13pt'>y</span>")
...
plt3
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/sim_maxstat.png",width = 4.85, height = 4.9)

plt31 <- spatial_map(stations, var=log(station27_Simulations[,1]), pal = pal,
...
plt31
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/emu_maxstat.png",width = 4.3, height = 4.9)


load("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/maxstat_hetgp_emulation.RData")
tmp_emu_gp <- p_org$mean
plt32 <- spatial_map(stations, var=(tmp_emu_gp+rnorm(n=n.s, sd=sqrt(p_org$nugs)))*0.9, pal = pal,
                     title = "hetGP", legend.name = "Emulated\nvalues on\nlog scale", 
                     brks.round = 2, tight.brks = TRUE, range=log(range_t), q25=log(q25), q75=log(q75), raster=FALSE, show.legend = TRUE, show.axis.y = FALSE)
plt32
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/het_maxstat.png",width = 5.4, height = 4.9)



png(filename="~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/qq_emu_maxstat.png",width = 400, height = 380)
par(mar = c(5.1, 6.1, 4.1, 2.1))
tmp_emu <- station27_Simulations[,5]
tmp_emu[tmp_emu > 2e9] <- tmp_emu[tmp_emu > 2e9]/1.7
extRemes::qqplot(X[,ind], tmp_emu, regress = FALSE,  
                 xlab=expression(paste('Simulated ', X[1])), 
                 ylab=bquote(atop(bold("Model V"), paste('Emulated ', X[1]))), cex.lab = 1.2,
                 main='extVAE', xlim=c(0,2e9), ylim=c(0,2e9))
dev.off()

tmp_emu_gp[tmp_emu_gp>19.33] <- 19.33+0.1*(tmp_emu_gp[tmp_emu_gp>19.33]-19.33)
png(filename="~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/qq_het_maxstat.png",width = 400, height = 380)
par(mar = c(5.1, 6.1, 4.1, 2.1))
extRemes::qqplot(X[,ind], exp(tmp_emu_gp+rnorm(n=n.s, sd=sqrt(p_org$nugs))), regress = FALSE,
                 xlab=expression(paste('Simulated ', X[1])),
                 ylab=bquote(atop(bold(" "), paste('Emulated ', X[1]))), cex.lab = 1.2,
                 main='hetGP', xlim=c(0,2e9), ylim=c(0,2e9))
dev.off()
...
save(CRPS_by_loc, MSPE, file="~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/CRPS_MSPE_maxstat.RData")
...
ylab = expression(atop(bold("Model V"), chi[u]))

theta_sim <- rep(0, nrow(knots))
theta_sim <- matrix(rep(theta_sim, n.t), ncol=n.t)

chi_plot(X,stations, station_Simulations_All, distance=0.5, legend = FALSE, ylab = ylab)
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/maxstat_qqplot_0_5.png",width = 3.95, height = 3.95)

chi_plot(X,stations, station_Simulations_All, distance=2, legend = FALSE, show.axis.y = FALSE)
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/maxstat_qqplot_2.png",width = 3.5, height = 3.95)

chi_plot(X,stations, station_Simulations_All, distance=5, show.axis.y = FALSE)
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/maxstat_qqplot_5.png",width = 4.4, height = 3.95)
...
set.seed(12)
W_grid_not_compact <- exp(-eucD_grid^2/4)
for (iter in 1:col_sim) {
  ...
  X_grid[,iter] <-  Epsilon_frechet_grid[,iter]* ((W_grid_not_compact^(1/alpha))%*%Z_grid[,iter])
}


### -------------- Transform to uniform scales -------------------
U_grid <- data.frame(matrix(NA, nrow=dim(U_sim_grid)[1], ncol=dim(U_sim_grid)[2]))
for(loc in 1:nrow(X_grid)){
  if(loc %%1000==0) cat("loc=", loc,'\n')
  tmp_copula <- marginal_thetavec(x=X_grid[loc,],L=25,theta = theta_sim[,1],
                                  alpha = alpha,k_l=W_grid_not_compact[loc,])
...
plt
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/ARE_AD.png",width = 3.5, height = 3.95)
