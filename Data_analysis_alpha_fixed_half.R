...
plt3 <- spatial_map(stations, var=original_data[,ind], pal = pal,
                    title = paste0('Mon_max temperature observed at time ', ind), legend.name = "°C", 
                    brks.round = 1, tight.brks = TRUE, range=range_obs, q25=q25, q75=q75, pt.size=0.4, raster=TRUE)
plt3
...
plt32 <- spatial_map(stations, var=mon_max_emulation_smooth, pal = pal,
                     title = paste0('Mon_max temperature smoothly emulated at time ', ind), legend.name = "°C", 
                     brks.round = 1, tight.brks = TRUE, range=range_obs, q25=q25, q75=q75, pt.size=0.4, raster=TRUE)
plt32
...
plt3 <- spatial_map(stations, var=X[,ind], pal = pal,
                    ...
                    brks.round = 2, tight.brks = TRUE, range=range_t, q25=q25, q75=q75, pt.size=0.4, raster=TRUE)
plt3
...
pal <- RColorBrewer::brewer.pal(9,"OrRd")
plt3 <- spatial_map(stations, var=X[,ind], pal = pal,
                    title = paste0('Observed replicate #', ind), legend.name = "Observed\n values", 
                    brks.round = 1, tight.brks = TRUE, range=range_t, q25=q25, q75=q75, pt.size=0.4, raster=TRUE)
plt3
# ggsave("/Users/LikunZhang/Desktop/img3.png",width = 5.5, height = 5)
...
plt3 <- spatial_map(stations, var=X[,ind], pal = pal,
                    title = paste0('Observed copula replicate #', ind), legend.name = "Observed\n values", 
                    brks.round = 1, tight.brks = TRUE, range=range_t, q25=q25, q75=q75, raster=TRUE)
plt3

plt31 <- spatial_map(stations, var=station100_Simulations[,floor(n.sim/2)], pal = pal,
                     title = paste0('Emulated coplua replicate #', ind), legend.name = "Emulated\n values", 
                     brks.round = 1, tight.brks = TRUE, range=range_t, q25=q25, q75=q75, raster=TRUE)
plt31

plt32 <- spatial_map(stations, var=as_array(y_star[,ind]), pal = pal,
                     title = paste0('Emulated coplua smooth replicate #', ind), legend.name = "Emulated\n values", 
                     brks.round = 1, tight.brks = TRUE, range=range_t, q25=q25, q75=q75, raster=TRUE)
plt32
...
### ------------------------------------------------------------------------------------------------------------------------------------------------------------
### ------------------------------------------------------------------------------------------------------------------------------------------------------------
mon_max_tmp <- fitted_gev_par$beta - fitted_gev_par$scale/(abs(fitted_gev_par$shape)*(station100_Simulations[,floor(n.sim/2)])^{abs(fitted_gev_par$shape)})
mon_max_tmp_smooth <- fitted_gev_par$beta - fitted_gev_par$scale/(abs(fitted_gev_par$shape)*(as_array(y_star[,ind]))^{abs(fitted_gev_par$shape)})
