...
load("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/HomGP.RData")
stations_holdout <- stations[holdout, ]
stations <- stations[-holdout, ]
...
eucD_holdout <- rdist(stations_holdout,as.matrix(knots))

W_holdout <- wendland(eucD_holdout,r=r)
dim(W_holdout)
W_holdout <- sweep(W_holdout, 1, rowSums(W_holdout), FUN="/")
...
#                                  control = list(fnscale=-1, trace=0, maxit=5000))
#                     initial_Z_t <- res$par
#                     if(any(initial_Z_t<0)) initial_Z_t[initial_Z_t<0] <- 0.1
...
#                                              control = list(fnscale=-1, trace=0, maxit=10000))
#                                 return(exp(res$par))})
#   if(res$convergence==1) {cat('-- maxit had been reached for time', iter, '\n'); better_Z_approx[, iter] <- Z_approx[,iter]} else{better_Z_approx[, iter] <- out}
# }
# 
# Z_approx <- better_Z_approx
better_Z_approx <- Z_approx
save(better_Z_approx, knots, file="./Z_approx_Gaussian.RData")
...
W_tensor <- torch_tensor(W,dtype=torch_float(), device = 'cpu')
...
# start_time <- Sys.time()
for (t in 1:niter) {
  if(t==3000) { learning_rate <- -5e-15; alpha_v <- 0.9}
  if(t==5000) { learning_rate <- -7e-15; alpha_v <- 0.9}
  if(t==20000) { learning_rate <- -7e-15; alpha_v <- 0.8}
  if(t==40000) { learning_rate <- -1e-14; alpha_v <- 0.6}
  if(t==80000) { learning_rate <- -1e-13; alpha_v <- 0.4}
  if(t==100000) { learning_rate <- -5e-13; alpha_v <- 0.4}
  if(t==120000) { learning_rate <- -5e-13; alpha_v <- 0.4}
  if(t==140000) { learning_rate <- -6e-13; alpha_v <- 0.4}
  if(t==300000) { learning_rate <- -1e-10; alpha_v <- 0.4}
  ...
  if(leak>0 & leak<=190) standardized$abs_()
...
loss - part3/(n.s*n.t)
plot(as_array(theta_t[,1]), xlab="index", ylab=expression(paste('CVAE ',theta)), ylim=c(0, max(as_array(theta_t[,16]))))
...
for(iter in 1:n.sim){
  ...
  ##Decoder
  station1_Simulations[, iter] <- rfrechet(n.s,shape=200, location = 0, scale = 1) * as_array((y_star[,1]))
  station50_Simulations[, iter] <- rfrechet(n.s,shape=200, location = 0, scale = 1) * as_array((y_star[,50]))
  station55_Simulations[, iter] <- rfrechet(n.s,shape=200, location = 0, scale = 1) * as_array((y_star[,55]))
  station70_Simulations[, iter] <- rfrechet(n.s,shape=200, location = 0, scale = 1) * as_array((y_star[,70]))
  station100_Simulations[, iter] <- rfrechet(n.s,shape=200, location = 0, scale = 1) * as_array((y_star[,100]))
}
save(better_Z_approx, n.sim, y_star, station1_Simulations, file="../simulation_VAE_results_Gaussian.RData" )


load("../simulation_VAE_results_Gaussian.RData"); X_org <- X; stations <- stations[-holdout,]
## -------- time 1 --------
ind <- 1
tmp_X <- X_org[,ind]#qnorm((1/X[,ind])^2)
range_t <- range(tmp_X)
q25 <- quantile(tmp_X, 0.25)
q75 <- quantile(tmp_X, 0.75)



library(ggtext)
ylab = paste0("<span style='font-size: 16pt'>**Model  I**</span><br><span style='font-size: 13pt'>y</span>")
pal <- RColorBrewer::brewer.pal(9,"OrRd")
plt3 <- spatial_map(stations, var=tmp_X, pal = pal,ylab = ylab,
                    title = paste0('Simulated replicate #', ind), legend.name = "Observed\n values", 
                    brks.round = 2, tight.brks = TRUE, range=range_t, q25=q25, q75=q75, raster=FALSE, show.legend = FALSE) +
        theme(axis.title.y = ggtext::element_markdown())
plt3
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/sim_GP.png",width = 4.85, height = 4.9)

tmp_emu <- qnorm((1/station1_Simulations[,floor(n.sim/2)])^2)
plt31 <- spatial_map(stations, var=tmp_emu, pal = pal,
...
plt31
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/emu_GP.png",width = 4.3, height = 4.9)


load("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/HomGP.RData");stations <- stations[-holdout,]
tmp_emu_gp <- emulation_GP[,ind]
plt32 <- spatial_map(stations, var=tmp_emu_gp, pal = pal,
...
plt32
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/het_GP.png",width = 5.4, height = 4.9)



ind <- 1

png(filename="~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/qq_emu_gp.png",width = 400, height = 380)
par(mar = c(5.1, 6.1, 4.1, 2.1))
extRemes::qqplot(tmp_X, tmp_emu, regress = FALSE,  
                 xlab=expression(paste('Simulated ', X[1])), 
                 ylab=bquote(atop(bold("Model I"), paste('Emulated ', X[1]))), cex.lab = 1.2,
                 main='extVAE')
dev.off()

load("~/Desktop/GEV-GP_VAE/HomGP.RData")
stations <- stations[-holdout, ]
png(filename="~/Desktop/GEV-GP_VAE/Figures/qq_het_gp.png",width = 400, height = 380)
par(mar = c(5.1, 6.1, 4.1, 2.1))
extRemes::qqplot(X[,1], emulation_GP[,1], regress = FALSE,
                 xlab=expression(paste('Simulated ', X[1])),
...
for(iter in 1:n.sim){
  ...
  y_star <- lrelu(W_holdout_alpha_tensor$mm(v_t)$add(b_8_holdout_tensor))
...
station_Simulations <- as_array(y_star)
...
save(CRPS_by_loc, MSPE, file="~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/CRPS_MSPE_GP.RData")
...
##Decoder
station_Simulations_All <- matrix(rfrechet(n.s*n.t,shape=200, location = 0, scale = 1), nrow=n.s) * as_array((y_star))
X_norm <- (1/X)^2
station_Simulations_norm <- (1/station_Simulations_All)^2

ylab = expression(atop(bold("Model I"), chi[u]))

library(ggh4x)
theta_sim <- as_array(theta_t)
chi_plot(X_norm, stations, station_Simulations_norm, distance=0.5, legend = FALSE, ylab = ylab, uniform = TRUE)
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/GP_qqplot_0_5.png",width = 3.95, height = 3.95)

chi_plot(X_norm, stations, station_Simulations_norm, distance=2, legend = FALSE, show.axis.y = FALSE, uniform = TRUE)
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/GP_qqplot_2.png",width = 3.5, height = 3.95)

chi_plot(X_norm, stations, station_Simulations_norm, distance=5, show.axis.y = FALSE, uniform = TRUE)
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/GP_qqplot_5.png",width = 4.4, height = 3.95)
...
eucD_grid <- rdist(pred_grid,as.matrix(knots))
...
for(loc in 1:nrow(station_Simulations_grid)){
  tmp_copula <- ecdf(station_Simulations_grid[loc,])
  U_sim_grid[loc, ] <- tmp_copula(station_Simulations_grid[loc,])
}
...
plot(u_vec, sqrt(Emp_ARE[,3]/pi), type='l', ylim=c(0,6))
...
eucD_grid <- rdist(pred_grid)
cov <- fields::Matern(eucD_grid, range=0.5, nu=5/2)
X_grid <- mvtnorm::rmvnorm(n=col_sim, sigma=cov, method='chol')
...
save(u_vec, Emp_ARE_ori, Emp_ARE, file="~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/ARE_GP.RData")

load("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/ARE_GP.RData")
...
plt
ggsave("~/Library/Mobile Documents/com~apple~CloudDocs/Desktop/GEV-GP_VAE/Figures/ARE_GP.png",width = 3.6, height = 3.95)
